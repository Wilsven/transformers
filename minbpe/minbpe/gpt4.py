...
def bpe(mergeable_ranks: dict, token: list[int], max_rank: int):
    """
    A helper function used in get_gpt4_merges() to reconstruct the merge forest.

    Args:
        mergeable_ranks (dict): A dictionary containing mergeable ranks.
        token (list[int]): A list of integers representing tokens.
        max_rank (int): The maximum rank value.

    Returns:
        list: A list of bytes representing the reconstructed merge forest.
    """
...
def recover_merges(mergeable_ranks: dict):
